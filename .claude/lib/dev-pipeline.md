...
work** from kanban intake to an open PR. It is the source of truth: `/que-task` and `/rework`
both reference it and **neither restates the steps**. `/que-task` runs it once per standalone
task and once per epic child; `/rework` runs the identical engine on re-queue.
...
10. **Teardown** per `worktree-workflow`: the pipeline leaves **no dangling worktree or sub-branch
    ...
    (`/task-merge` for a standalone task; the epic DAG's auto-merge into the integration branch
    for an epic child).
...
The engine **never** sets `merged` / `deployed` / `complete` — those are human / `/task-merge` /
