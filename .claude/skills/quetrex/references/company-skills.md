...
```sh
curl -sS -X POST "$QUETREX_API_URL/api/companies/$QUETREX_COMPANY_ID/skills/import" \
...
```sh
curl -sS -X POST "$QUETREX_API_URL/api/companies/$QUETREX_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $QUETREX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$QUETREX_API_URL/api/companies/$QUETREX_COMPANY_ID/skills" \
  -H "Authorization: Bearer $QUETREX_API_KEY"
```
...
```sh
curl -sS "$QUETREX_API_URL/api/companies/$QUETREX_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $QUETREX_API_KEY"

curl -sS "$QUETREX_API_URL/api/companies/$QUETREX_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $QUETREX_API_KEY"
```
...
```sh
curl -sS -X POST "$QUETREX_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $QUETREX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$QUETREX_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $QUETREX_API_KEY"
```
...
```sh
curl -sS -X POST "$QUETREX_API_URL/api/companies/$QUETREX_COMPANY_ID/agents" \
  -H "Authorization: Bearer $QUETREX_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in Quetrex Cloud runtime skills are still added automatically when required by the adapter.
