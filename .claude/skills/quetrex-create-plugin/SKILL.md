---
name: quetrex-create-plugin
description: >
  Create new Quetrex Cloud plugins with the current alpha SDK/runtime. Use when
...
# Create a Quetrex Cloud Plugin

Use this skill when the task is to create, scaffold, or document a Quetrex Cloud plugin.
...
```bash
pnpm --filter @quetrex/create-quetrex-plugin build
node packages/plugins/create-quetrex-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Quetrex Cloud repo, pass `--sdk-path /absolute/path/to/quetrex-sdk/`:

```bash
pnpm --filter @quetrex/create-quetrex-plugin build
node packages/plugins/create-quetrex-plugin/dist/index.js @acme/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/quetrex/packages/plugins/sdk
```
...
- is installed into Quetrex Cloud from an absolute local path during development
