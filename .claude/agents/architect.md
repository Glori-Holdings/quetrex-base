...
2. **Read the verify chain.** Read `./.quetrex/verify.json` and copy its ordered `.verify[]` array verbatim into your plan's `verify` field. Do NOT invent commands or reorder them. If the file is absent, set `verify` to `[]` and add a `notes` entry: `"verify.json missing — run /q-init"`.
