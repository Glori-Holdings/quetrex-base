...
6. **A chain command is never killed by a tool timeout and never retried.** Run every verify-chain command in the foreground with an explicit long timeout (`timeout: 900000` on the Bash tool — a real app's suite, build, or E2E takes minutes), wait for it, and record the exit code you observed. If a run is cut off by a timeout, that is one FAILED run to report — it is not a reason to launch the command again. One command, one run, one exit code.
