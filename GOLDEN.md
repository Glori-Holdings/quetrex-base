...
| 2 | `/quetrex-setup:init` in a fresh clone | `.quetrex/project.json` + `.quetrex/verify.json` are **committed** (not merely on disk); `enabledPlugins` carries the engine unpinned; `/quetrex-setup:doctor` reports every check green | AUTO |
